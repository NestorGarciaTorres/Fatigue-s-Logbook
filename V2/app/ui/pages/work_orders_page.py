...
class WorkOrdersPage(BasePage):
    ...
    def __init__(self, context: AppContext, parent=None):
        ...
        actions.addWidget(QLabel("Mostrar"))
        ...
        hint = QLabel(
            "Solo Fatiga y Rotary tienen boton para comenzar la prueba. Las "
            "ordenes de Torsion y Quasi se registran igual, pero esas "
            "bitacoras siguen dando de alta sus pruebas por su cuenta."
        )
    ...
    def _fill_row(self, row: int, order: WorkOrder) -> None:
        tipo = self._item(TYPE_LABELS.get(order.test_type, order.test_type))
        self.table.setItem(row, 0, tipo)
        self.table.setItem(row, 1, self._item(order.test_batch))
        self.table.setItem(row, 2, self._item(order.customer))
        self.table.setItem(row, 3, self._item(str(order.qty_samples)))
        self.table.setItem(row, 4, self._item(order.requester))

        self.table.setItem(
            row, 5, self._item(dates.display(order.created_at) if order.created_at
                               else "")
        )
        ...
        if order.is_started:
            ...
            if order.started_test_id:
                estado.setToolTip(
                    f"Registro #{order.started_test_id} en la bitacora de "
    ...
    def _action(self, order: WorkOrder):
        ...
        if order.test_type not in STARTABLE_TEST_TYPES:
            ...
            return self._note(
                "  alta en su bitacora  ",
                f"{label} todavia no se comienza desde aqui: da de alta la "
                f"prueba en la bitacora de {label}.",
            )
    ...
    def edit_order(self) -> None:
        ...
        if order.is_started:
            QMessageBox.information(
                ...
                "registro de la bitacora, no aqui: cambiarlos ahora dejaria la "
    ...
    def start_test(self, order: WorkOrder) -> None:
        ...
        else:                                  # pragma: no cover - boton inactivo
            return
        ...
        QMessageBox.information(
            ...
            f"Se creo el registro #{dialog.created_id} en la bitacora de "
