...
class RigsTab(QWidget):
    ...
    def __init__(self, context: AppContext, parent=None):
        ...
        info = QLabel(
            ...
            "en las bitacoras, el dashboard y el reporte de Excel. Solo los "
...
class CodesTab(QWidget):
    ...
    def __init__(self, context: AppContext, parent=None):
        ...
        info = QLabel(
            "El Test Batch debe seguir el formato 6 digitos + clave + 2 digitos "
    ...
    def add_code(self) -> None:
        ...
        if len(code) != 3 or not code.isalpha():
            QMessageBox.warning(
                self, "Clave invalida", "La clave debe ser de tres letras."
            )
            return
    ...
    def remove_code(self) -> None:
        ...
        confirm = QMessageBox.question(
            self, "Confirmacion", f"Eliminar la clave {code.code}?"
        )
    ...
    def rename(self, row: int) -> None:
        ...
        if usage:
            answer = QMessageBox.question(
                ...
                f"Renombrarlo aqui cambia solo la lista: esas muestras seguiran "
    ...
    def remove_mode(self) -> None:
        ...
        if usage:
            QMessageBox.information(
                self,
                "El modo esta en uso",
                f"'{mode.label}' esta capturado en {usage:,} muestras, asi que "
                f"no se elimina: dejaria esos registros con un valor sin "
        ...
        confirm = QMessageBox.question(
            self, "Confirmacion", f"Eliminar el modo '{mode.label}'?"
        )
        if confirm != QMessageBox.StandardButton.Yes:
            return
...
class RequestersTab(QWidget):
    ...
    def __init__(self, context: AppContext, parent=None):
        ...
        info = QLabel(
            ...
            "laboratorio y hay que darlas de alta aqui."
        )
        info.setProperty("muted", "true")
        info.setWordWrap(True)
        layout.addWidget(info)
    ...
    def rename(self, row: int) -> None:
        ...
        if usage:
            answer = QMessageBox.question(
                self,
                "El solicitante tiene ordenes",
                f"{usage:,} Work Orders quedaron capturadas a nombre de "
                f"'{person.name}'.\nRenombrarlo aqui cambia solo la lista: "
                f"esas ordenes seguiran mostrando el nombre anterior."
                f"\n\nContinuar?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No,
            )
    ...
    def remove_requester(self) -> None:
        ...
        if usage:
            QMessageBox.information(
                self,
                "El solicitante tiene ordenes",
                f"'{person.name}' aparece en {usage:,} Work Orders, asi que no "
        ...
        confirm = QMessageBox.question(
            self, "Confirmacion", f"Eliminar a '{person.name}'?"
        )
        if confirm != QMessageBox.StandardButton.Yes:
            return
...
class DatabaseTab(QWidget):
    def __init__(self, context: AppContext, parent=None):
        ...
        self.path_label = QLabel(str(context.config.database))
        ...
        change = QPushButton("Cambiar base de datos")
        change.clicked.connect(self.change_database)
        buttons.addWidget(change)

        test = QPushButton("Probar conexion")
        ...
        history = QPushButton("Historial completo")
    ...
    def refresh(self) -> None:
        self.path_label.setText(str(self.context.config.database))
    ...
    def change_database(self) -> None:
        ...
        QMessageBox.information(
            self, "Listo",
            "La app quedo apuntando a la nueva base. "
            "Regresa al menu para recargar las bitacoras.",
        )
        self.refresh()
    ...
    def test_connection(self, quiet: bool = False) -> None:
        try:
            ...
            if not quiet:
                QMessageBox.critical(self, "Sin conexion", str(error))
            return

        self.status_label.setText(f"Conectada. {len(tables)} tablas.")
        if not quiet:
            QMessageBox.information(
                self, "Conexion correcta",
