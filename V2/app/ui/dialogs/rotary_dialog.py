...
class RotaryDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        self.rig = closed_combo(self.catalogs.rig_names("rotary"))
        ...
        for slot in range(SAMPLE_SLOTS):
            ...
            status = closed_combo(status_labels)
    ...
    def _update_sample_slots(self) -> None:
        ...
        Igual que en Fatiga: las nueve se muestran siempre, y lo que se senala
        es la incoherencia.
        """
        ...
        for revs, status, mode in zip(self.revs, self.statuses,
            ...
            samples.append(
                RotarySample(
                    ...
                    status=status.currentText().strip() or EMPTY_RIG,
