...
class WorkOrderDialog(QDialog):
    ...
    def _update_hint(self) -> None:
        ...
        self.hint.setText(
            "Test Batch: 6 digitos + clave + 2 digitos. "
            f"Claves de {TYPE_LABELS.get(key, key)}: "
            + (", ".join(codes) if codes else "sin claves en el catalogo")
        )
    ...
    def _save(self) -> None:
        ...
        try:
            validation.validate_required(order.customer, order.qty_samples)
            validation.validate_test_batch(
                order.test_batch, self.catalogs.codes(order.test_type)
            )
            validation.validate_unique_batch(
                order.test_batch,
                self.repository.batch_exists(
                    order.test_batch, exclude_id=order.id
                ),
        ...
        if not order.requester:
            QMessageBox.warning(
                self, "Falta el solicitante",
                "Elige quien solicita la prueba. Si la lista esta vacia, "
        ...
        try:
            if self.editing:
                self.repository.update(order, author)
            else:
