...
class QuasiLogbook:
    def __init__(self, master, main_root_ref):
        ...
        self.root.title("Pruebas de | Quasi | finalizadas")
    ...
    # ----- CONFIGURACIÓN DE ESTILOS Y TIPOS DE LETRA
        ...
        self.form_config = ["quasi_tests", "QUASI", "Quasi Codes", "#6ec6ff", "info", "Quasi Rigs"]

        ttk.Label(self.root, text="--- PRUEBAS DE | QUASI | ACTUALMENTE CORRIENDO ---", font=("Roboto",24,"bold"), bootstyle="info").pack(pady=10)
    ...
    # -FRAME PARA LOS COMPONENTES DE FILTROS
        ...
        Button(frame_filters, text="Nuevo registro",bootstyle="info-outline", command=self.open_generic_form, cursor="hand2").pack(side="left", padx=(0,50))
        
        ttk.Label(frame_filters, text="Mes: ", font=font_components, width=4).pack(side="left", padx=5)
        self.month_combo = ttk.Combobox(frame_filters, textvariable=self.selected_month,values=months, width=12, font=font_components, bootstyle="info")
        ...
        self.year_combo = ttk.Combobox(frame_filters, textvariable=self.selected_year,values=years, width=6, font=font_components, bootstyle="info")
        ...
        try:
            ...
            self.coldata = [
                                ...
                                {"text": "Quasi Rig","anchor": "center", "width": 150},
                            ]
        ...
        # ----- Evento en tree al dar doble click sobre un registro
            ...
            self.new_quasi_window = None
        except Exception as e:
            print("Ocurrio un error: ",e)
            input("Presiona enter para salir")
        
        
    def fetch_quasi_data(self):
    ...
    def open_generic_form(self):
        if self.new_quasi_window is None or not self.new_quasi_window.winfo_exists():
            self.new_quasi_window = tk.Toplevel(self.root)
            conn = sqlite3.connect("./db/test_records.db")
            GenericForm(self.new_quasi_window, self, None, self.refresh_and_show, conn, False, self.form_config)
    
    def refresh_and_show(self):
        new_data = self.fetch_quasi_data()
