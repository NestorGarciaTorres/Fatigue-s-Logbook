...
# Rojo del banco en mantenimiento. No es DANGER, y no por gusto: DANGER sobre
# la franja alterna de la tabla se queda en 2.49:1, asi que el numero dentro
# del chip deja de leerse justo en una fila de cada dos. Este es el rojo mas
# cercano que llega a 3:1 sobre LOS TRES fondos de fila. Lo que cuesta la
# eleccion esta anotado en app/ui/widgets/sample_chips.py.
MAINTENANCE = "#FE9296"
...
QPushButton[accent="maintenance"] {{ border-color: {MAINTENANCE}; color: {MAINTENANCE}; }}
QPushButton[accent="maintenance"]:hover {{ background-color: {MAINTENANCE}; color: {BACKGROUND}; }}
...
QPushButton[accent="maintenance"]:disabled,
...
/* Campo que impide guardar (form_guard.show_issues). Va despues del foco para
   ganarle: el campo marcado recibe el foco y tiene que seguir viendose rojo.
   Es el rojo de 'revisar', no un color nuevo. */
QLineEdit[invalid="true"], QComboBox[invalid="true"], QDateEdit[invalid="true"] {{
    border: 2px solid {DANGER};
}}
/* Aviso de datos nuevos de otro equipo (widgets/changes_bar.py). Informa, no
   alarma: nada esta mal, solo hay algo mas reciente que lo que se ve. */
QFrame[changes="true"] {{
    background-color: {SURFACE};
    border: 1px solid {BORDER};
...
/* Con Fusion la lista se abria centrada sobre el campo e ignoraba
   maxVisibleItems: la de clientes media 516 px y crecia con el catalogo.
   combobox-popup: 0 la abre debajo del campo, con tope de filas y barra. */
QComboBox {{
    combobox-popup: 0;
}}
...
/* Sin esto, al dejar el modo centrado las filas bajaban de 27 a 19 px. */
