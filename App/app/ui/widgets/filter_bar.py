...
class DateEdit(QDateEdit):
    """QDateEdit que no cambia de fecha con la rueda si no tiene el foco.

    Con la politica de siempre (WheelFocus), pasar la rueda por encima de una
    fecha la movia un dia aunque se estuviera escribiendo en otro campo. En un
    formulario desplazable eso era cambiar el inicio de prueba al bajar.
    """
    ...
    def wheelEvent(self, event) -> None:
        if not self.hasFocus():
            # Ignorado, Qt lo pasa al padre: el area desplazable se mueve.
            event.ignore()
            return
