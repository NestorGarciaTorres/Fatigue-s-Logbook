...
def _estado_pieza(record: RigMaintenance, sample) -> str:
    ...
    Son cuatro casos y no dos: volvio a su banco, sigue esperando, la llevaron
    a otro banco para seguir probandola, o el mantenimiento cerro sin
    devolverla.
    """
    if sample.restored:
        return "  (volvió a su banco)"
    if sample.released_date:
        return f"  (siguió en otro banco el {dates.display(sample.released_date)})"
    if record.is_open:
...
class MaintenanceHistoryDialog(QDialog):
    ...
    def __init__(self, records: list[RigMaintenance], title: str | None = None,
        ...
        # ActionRole: exportar no cierra la ventana.
        self.export_button = buttons.addButton(
            "Exportar a Excel", QDialogButtonBox.ButtonRole.ActionRole
        )
    ...
    def export(self) -> None:
        """Los mismos periodos que ensenia la ventana, a Excel."""
        registros = self._records
        save_report(
            self, len(registros), suggested_filename("Mantenimiento_bancos"),
            lambda path: export_maintenance(registros, path),
    ...
    def populate(self, records: list[RigMaintenance]) -> None:
        ...
        for row, record in enumerate(records):
            # Detenidas hoy: solo las tiene un periodo que sigue abierto.
            paradas = ([s for s in record.samples if s.is_held]
                       if record.is_open else [])
            dias = record.days()
            ...
            if paradas:
                self.table.item(row, HEADERS.index("Piezas detenidas")).setText(
                    f"{len(record.samples)}  ({len(paradas)} detenida"
                    + ("s)" if len(paradas) != 1 else ")")
                )
