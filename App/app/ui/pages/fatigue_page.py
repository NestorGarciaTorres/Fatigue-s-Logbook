"""Bitacora de Fatiga: pruebas en curso y finalizadas.

Unifica ``logbooks/fatigue_curr_logbook.py`` y
``logbooks/fatigue_cmplt_logbook.py``, que eran dos ventanas separadas con la
misma consulta y la misma tabla, distintas solo en una columna y un WHERE.
"""

from __future__ import annotations

from PySide6.QtCore import Qt, Signal
from PySide6.QtGui import QKeySequence, QShortcut
from PySide6.QtWidgets import (
    QDialog,
    QFileDialog,
    QHBoxLayout,
    QMessageBox,
    QPushButton,
    QTabWidget,
    QVBoxLayout,
    QWidget,
)

from app.context import AppContext
from app.models import FINISHED, ONGOING, FatigueTest
from app.services import duration, filtering, maintenance
from app.services.excel_export import (
    export_fatigue,
    export_fatigue_ongoing,
    suggested_filename,
)
from app.ui.dialogs.cycles_dialog import CyclesDialog
from app.ui.dialogs.fatigue_dialog import FatigueDialog
from app.ui.models.table_models import FatigueTableModel
from app.ui.report_export import filter_period, save_report
from app.ui.widgets.common import StatCard, heading
from app.ui.widgets.filter_bar import FilterBar
from app.ui.widgets.legend import ActiveFilters, Legend
from app.ui.widgets.record_table import RecordTable


class FatigueTab(QWidget):
    """Una de las dos pestanas. La logica es identica; cambia el estatus."""

    def __init__(self, context: AppContext, status: str, parent=None):
        super().__init__(parent)
        self.context = context
        self.status = status
        self.is_finished = status == FINISHED
        self._records: list[FatigueTest] = []

        layout = QVBoxLayout(self)
        layout.setSpacing(10)

        # --- acciones ----------------------------------------------------
        actions = QHBoxLayout()

        # Ya no hay alta directa: una prueba de fatiga nace de una Work Order.
        # El formulario es el mismo, pero se abre desde esa pantalla.
        self.export_button = QPushButton("Exportar a Excel")
        self.export_button.setProperty("accent", "success")
        self.export_button.setShortcut("Ctrl+E")
        self.export_button.setToolTip("Ctrl+E")
        self.export_button.clicked.connect(self.export)
        actions.addWidget(self.export_button)
        # En las dos pestanias. Empezo solo en curso, que fue el reporte que se
        # pidio; el de finalizadas se saca con los filtros de fecha puestos.

        # Abrir un registro solo se podia con doble clic, sin nada que lo
        # dijera. El boton lo pone a la vista y de paso da el atajo.
        self.open_button = QPushButton("Abrir registro")
        self.open_button.setProperty("accent", "secondary")
        self.open_button.setToolTip("Doble clic sobre la fila, o Enter")
        self.open_button.setEnabled(False)
        self.open_button.clicked.connect(lambda: self.table.open_selected())
        actions.addWidget(self.open_button)

        # Anotar ciclos es lo que mas se repite, y con el registro completo era
        # buscar la pieza entre 36 campos. Solo en curso: una prueba
        # finalizada ya no avanza.
        self.cycles_button = QPushButton("Capturar ciclos")
        self.cycles_button.setProperty("accent", "secondary")
        self.cycles_button.setShortcut("F2")
        self.cycles_button.setToolTip(
            "Solo los ciclos de las piezas que están corriendo (F2)"
        )
        self.cycles_button.setEnabled(False)
        self.cycles_button.clicked.connect(lambda: self.capture_cycles())
        self.cycles_button.setVisible(not self.is_finished)
        actions.addWidget(self.cycles_button)

        # Vista compacta (chips) frente a las 28 columnas de antes.
        self.view_toggle = QPushButton("Ver columnas completas")
        self.view_toggle.setProperty("accent", "secondary")
        self.view_toggle.setCheckable(True)
        self.view_toggle.setToolTip(
            "Alterna entre los chips de muestras y una columna por rig y por "
            "ciclos"
        )
        self.view_toggle.toggled.connect(self._toggle_view)
        actions.addWidget(self.view_toggle)

        actions.addStretch(1)
        layout.addLayout(actions)

        # Las tarjetas van en su propia fila. Junto a los botones, la fila
        # pedia 1,650 px de minimo y la bitacora nacia mas ancha que el
        # escritorio de un portatil.
        cards = QHBoxLayout()
        cards.addStretch(1)
        self.card_tests = StatCard("Pruebas", "0")
        self.card_samples = StatCard("Piezas probadas", "0")
        self.card_cycles = StatCard("Ciclos acumulados", "0")
        for card in (self.card_tests, self.card_samples, self.card_cycles):
            cards.addWidget(card)
        layout.addLayout(cards)

        # --- filtros -----------------------------------------------------
        self.filters = FilterBar(show_status=False, show_wo=True)
        self.filters.changed.connect(self.apply_filters)
        layout.addWidget(self.filters)

        self.active_filters = ActiveFilters()
        self.active_filters.removed.connect(self.filters.clear_field)
        self.active_filters.setVisible(False)
        layout.addWidget(self.active_filters)

        # --- tabla -------------------------------------------------------
        self.model = FatigueTableModel(
            context.catalogs, show_end_date=self.is_finished
        )
        self.table = RecordTable(self.model)
        self.table.recordActivated.connect(self.open_record)
        self.table.clearFiltersRequested.connect(self.filters.clear)
        self.table.selectionModel().selectionChanged.connect(
            lambda *_: self._update_actions()
        )
        if not self.is_finished:
            self.table.add_menu_action("Capturar ciclos", self.capture_cycles,
                                       "F2")
        layout.addWidget(self.table, 1)

        # La antiguedad solo se marca en la pestana de pruebas abiertas.
        self.legend = Legend(show_days=True, show_wo=not self.is_finished,
                             show_maintenance=True)
        layout.addWidget(self.legend)

    # --- datos -----------------------------------------------------------
    def refresh(self) -> None:
        self._records = self.context.fatigue.list(self.status)
        historial = self.context.fatigue.list()

        # El mantenimiento va antes que los umbrales: los dias en curso se
        # miden descontando lo que la prueba estuvo parada, y el historial con
        # el que se calibra el semaforo tiene que medirse igual.
        mantenimientos = self.context.maintenance.list()
        detenidas = maintenance.stopped_by_test(historial, mantenimientos)
        self.model.set_maintenance(
            detenidas, maintenance.paused_slots(mantenimientos)
        )

        # Umbrales del semaforo, calculados del historial de esta bitacora.
        thresholds = duration.DurationThresholds.from_history(
            duration.history_durations(historial, detenidas)
        )
        self.model.set_thresholds(thresholds)
        self.legend.set_thresholds(thresholds)

        self.filters.set_customers(self.context.catalogs.customer_names())
        self.filters.set_rigs(self.context.catalogs.rig_names("fatigue"))
        self.apply_filters()

    def apply_filters(self) -> None:
        active = self.filters.filters()
        visible = filtering.apply(self._records, active)
        self.table.set_records(visible, filtered=not active.is_empty())
        self.filters.set_result_count(len(visible), len(self._records))
        self.active_filters.update_from(active)

        self.card_tests.set_value(len(visible))
        self.card_samples.set_value(sum(t.qty_samples for t in visible))
        self.card_cycles.set_value(sum(t.total_cycles for t in visible))

    def _toggle_view(self, full_columns: bool) -> None:
        self.table.set_compact(not full_columns)
        self.view_toggle.setText(
            "Ver vista compacta" if full_columns else "Ver columnas completas"
        )

    # --- acciones --------------------------------------------------------
    def _update_actions(self) -> None:
        seleccionado = self.table.selected_record() is not None
        self.open_button.setEnabled(seleccionado)
        self.cycles_button.setEnabled(seleccionado and not self.is_finished)

    def _paused_for(self, record: FatigueTest) -> dict:
        """Las piezas de esta prueba detenidas por mantenimiento, por numero."""
        return {
            slot: registro
            for (test_id, slot), registro in self.model.paused_slots.items()
            if test_id == record.id
        }

    def capture_cycles(self, record: FatigueTest | None = None) -> None:
        """Abre la captura de ciclos de la prueba seleccionada."""
        record = record if record is not None else self.table.selected_record()
        if record is None or self.is_finished:
            return
        dialog = CyclesDialog(self.context.fatigue, record,
                              paused=self._paused_for(record), parent=self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def open_record(self, record: FatigueTest) -> None:
        # Las piezas de esta prueba que estan paradas por mantenimiento, por
        # numero de pieza. El modelo ya las tiene cargadas del refresco.
        detenidas = {
            slot: registro
            for (test_id, slot), registro in self.model.paused_slots.items()
            if test_id == record.id
        }
        dialog = FatigueDialog(
            self.context.fatigue, self.context.catalogs, self.context.audit,
            test=record, read_only=self.is_finished, paused=detenidas,
            # Los bancos en mantenimiento no se ofrecen para poner una pieza.
            unavailable_rigs=maintenance.rigs_in_maintenance(
                self.context.maintenance.open_records()
            ),
            parent=self,
        )
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.refresh()

    def export(self) -> None:
        """Lo que se ve, en el orden en que se ve: los filtros deciden."""
        records = self.table.visible_records()
        colores = self.context.catalogs.colors()

        if self.is_finished:
            periodo = filter_period(self.filters)
            save_report(
                self, len(records), suggested_filename("Fatigas_Finalizadas"),
                lambda path: export_fatigue(records, colores, path,
                                            finished=True, period=periodo),
            )
            return

        save_report(
            self, len(records), suggested_filename(),
            lambda path: export_fatigue_ongoing(
                records, colores, path,
                # Las piezas paradas por mantenimiento, ya cargadas por el
                # refresco: el reporte impreso tiene que distinguirlas de una
                # suspension igual que la pantalla.
                paused=self.model.paused_slots,
            ),
        )


class FatiguePage(QWidget):
    # Igual que BasePage: recargar con F5 pone al dia el aviso de datos nuevos.
    refreshed = Signal()

    def __init__(self, context: AppContext, parent=None):
        super().__init__(parent)
        self.context = context

        layout = QVBoxLayout(self)
        layout.addWidget(heading("Bitácora de Fatiga"))

        self.tabs = QTabWidget()
        self.ongoing = FatigueTab(context, ONGOING)
        self.finished = FatigueTab(context, FINISHED)
        self.tabs.addTab(self.ongoing, "En curso")
        self.tabs.addTab(self.finished, "Finalizadas")
        self.tabs.currentChanged.connect(self._on_tab_changed)
        layout.addWidget(self.tabs, 1)

        back = QPushButton("Regresar al menú")
        back.setProperty("accent", "secondary")
        back.setToolTip("Esc")
        back.clicked.connect(self._go_back)
        row = QHBoxLayout()
        row.addWidget(back, 0, Qt.AlignmentFlag.AlignLeft)
        row.addStretch(1)
        layout.addLayout(row)

        self._back_callback = None

        # Los mismos atajos que en el resto de pantallas. Esta pagina no
        # hereda de BasePage --lleva pestanias-- asi que los pone aparte.
        for sequence, slot in (
            (QKeySequence(Qt.Key.Key_Escape), self._go_back),
            (QKeySequence.StandardKey.Refresh, self.reload),
            (QKeySequence.StandardKey.Find, self._focus_search),
        ):
            atajo = QShortcut(sequence, self)
            atajo.setContext(Qt.ShortcutContext.WidgetWithChildrenShortcut)
            atajo.activated.connect(slot)

    def _focus_search(self) -> None:
        self.tabs.currentWidget().filters.focus_search()

    def set_back_callback(self, callback) -> None:
        self._back_callback = callback

    def _go_back(self) -> None:
        if self._back_callback:
            self._back_callback()

    def _on_tab_changed(self, index: int) -> None:
        self.tabs.widget(index).refresh()

    def refresh(self) -> None:
        self.tabs.currentWidget().refresh()

    def reload(self) -> None:
        """Recarga a peticion del usuario, y lo anuncia."""
        self.refresh()
        self.refreshed.emit()
