...
### El formulario en una pantalla chica
Todos los formularios de captura —**Fatiga**, **Rotary**, **Torsión**, **Quasi**
y **Work Orders**— se abren con el alto que necesitan, **sin pasar del de la
pantalla**. Si no caben enteros:
- el título y la fila de botones (*Guardar cambios*, *Cancelar*, *Finalizar
  prueba*…) **quedan fijos y siempre a la vista**;
- lo de en medio —*Datos generales* y las nueve piezas— se desplaza con la
  rueda o con la barra de la derecha;
- al avanzar con Tab, el formulario se desplaza solo hasta el campo con el
  foco.

En un monitor donde caben enteros se ven como siempre, sin barra. Work Order,
Torsión y Quasi son cortos y caben en cualquier portátil; la barra solo les
aparece con una pantalla muy baja o con la escala de Windows alta, pero se
comportan igual que los largos.

La rueda del ratón **no cambia el valor de un desplegable o de una fecha que no
se ha seleccionado**: al pasar por encima, desplaza el formulario. Para elegir
con la rueda, primero hay que hacer clic en el campo.

Las **listas desplegables** se abren debajo del campo y enseñan **10 opciones**
como máximo; el resto se alcanza con su barra.

> Antes el formulario de Fatiga exigía **967 px** de alto y el de Rotary 850.
> Un portátil de 1366×768 deja unos 728, y un 1920×1080 al 150 % unos 680: la
> fila de *Guardar* quedaba debajo del borde de la pantalla y no había forma de
> llegar a ella. Las listas, por su parte, se abrían centradas sobre el campo y
> medían el catálogo entero: la de clientes, 516 px, y crecía con cada cliente
> nuevo. Ahora mide 262.

### Capturar ciclos sin abrir el registro
En *Fatiga → En curso*, selecciona una prueba y pulsa **Capturar ciclos**. El
botón también está en el menú del botón derecho, y el atajo es **F2**. Se abre
un cuadro con una fila por pieza: su banco, la lectura anterior y el campo de
ciclos nuevos.
- **Solo se capturan las piezas que están corriendo** (con banco y sin
  resultado). La terminada, la suspendida y la detenida por mantenimiento se
  ven apagadas, con el motivo al lado; sus ciclos, si hay que corregirlos, se
  corrigen desde el registro completo.
- El campo trae la lectura anterior seleccionada: se escribe encima, **Tab**
  pasa a la siguiente pieza y **Enter** guarda.
- **Dejar un campo vacío conserva la lectura anterior.**
- Una lectura **menor que la anterior** se avisa en rojo, pero se permite: los
  contadores solo suben, así que casi siempre es un dígito de menos, aunque
  también puede ser la corrección de una lectura mal anotada.
- Si otro equipo guardó algo en esa prueba con el cuadro abierto, se combina
  igual que en los formularios.

En *Finalizadas* el botón no aparece: esas pruebas ya no avanzan.

### Qué falta para guardar, marcado en el formulario
Al pulsar *Guardar* o *Finalizar prueba* con datos que faltan o que no son
válidos, el formulario ya no abre un cuadro con el primer error:
- **cada campo con problema se marca con borde rojo**, y al pasar el cursor
  explica qué le pasa;
- **bajo el título, un aviso dice cuántos son y cuáles** (enumera hasta seis y
  resume el resto);
- la vista baja sola hasta el primero y le pone el foco;
- **en cuanto corriges un campo, pierde la marca** y el aviso se actualiza; al
  corregirlos todos, desaparece.

Aplica a Fatiga, Rotary, Torsión, Quasi y Work Orders.

> Antes era un cuadro con un solo error: se cerraba, se corregía ese campo y al
> volver a guardar aparecía el siguiente. Para finalizar sí se enumeraban todos
> los faltantes, pero en un texto que al cerrarlo desaparecía, y había que
> recordarlos para buscarlos entre nueve piezas.
...
**Una pieza detenida puede seguir su prueba en otro banco.** Basta con abrir el
registro y asignarle un Test Rig distinto. Al guardar, el apunte anota **el
día en que se movió** (`released_date`) y desde ese día:

- el chip deja el rojo con trama y toma el color de su banco nuevo, en la vista
  compacta y en la de columnas completas;
- a su prueba se le dejan de descontar días, aunque el banco viejo siga parado;
- al terminar el mantenimiento no se la devuelve al banco viejo, aunque después
  se haya vuelto a quedar sin banco por otro motivo;
- el historial dice «siguió en otro banco el …» en vez de «sigue detenida».

Antes el apunte solo sabía si la pieza había vuelto a su banco al cerrar, así
que una pieza corriendo ya en otro banco seguía pintándose como detenida hasta
que alguien cerrara el mantenimiento.

**El día en que se movió se captura.** Al asignarle otro banco a una pieza
detenida, en su recuadro aparece la fila **Movida el**:
- por omisión trae la fecha de hoy;
- no admite fechas anteriores al inicio del mantenimiento ni futuras;
- esa fecha es la que se guarda, y los días detenida terminan ahí.

Antes se tomaba el día en que se guardaba el cambio, y un cambio capturado días
después seguía contando esos días como detenida. Cerrar el mantenimiento no
rellena la fecha: así «se movió a otro banco» y «no se repuso al cerrar» no se
confunden aunque pasen el mismo día.

**Un banco en mantenimiento no se ofrece en el campo Test Rig.** Desaparece de
las opciones del formulario de fatiga mientras su periodo esté abierto —al
comenzar una prueba desde Work Orders, al abrir un registro desde la bitácora y
desde la tarjeta del banco— y vuelve al terminar el mantenimiento. Una pieza que
ya tenía ese banco anotado (por ejemplo, una que falló ahí) conserva su valor:
se oculta la opción, no se borra el dato.
...
### Que no se pierdan datos sin que nadie lo note
Tres protecciones que no se ven mientras todo va bien.

**Dos equipos editando el mismo registro.** Si abres un registro y, mientras lo
tienes abierto, alguien en otra computadora guarda cambios en ese mismo
registro, al pulsar *Guardar*:
- Si cambiaron **campos distintos** —por ejemplo, él actualizó los ciclos de
  la pieza 1 y tú el resultado de la pieza 2—, se guardan los dos cambios, sin
  preguntar nada.
- Si los dos cambiaron **el mismo campo** a valores distintos, la app lo dice:
  qué campo, qué pusiste tú, qué quedó guardado y quién lo guardó y cuándo.
  Eliges **Guardar los míos**, **Dejar los guardados** o **Volver al
  formulario** sin guardar nada. El resto de tus cambios se guarda en
  cualquier caso.
- Si otro equipo **eliminó** el registro, la app avisa y no guarda.

Aplica a Fatiga, Rotary, Torsión, Quasi y Work Orders, también al pulsar
*Finalizar prueba*.

> Antes, guardar escribía el registro entero tal como estaba en el formulario,
> y ganaba quien guardara al último. Lo del otro equipo desaparecía sin aviso.
> El caso más delicado era una Work Order: si alguien la comenzaba mientras
> otra persona corregía sus comentarios, guardar la devolvía a *Pendiente*.
> Tampoco las piezas que el mantenimiento saca de su banco se vuelven a poner
> ahí por guardar un formulario que se abrió antes.

**Cambios sin guardar.** *Cancelar*, la tecla Esc o la X de la ventana
preguntan antes de cerrar **solo si cambiaste algo**: *Descartar cambios* o
*Seguir editando*. Si solo abriste el registro para mirarlo, cierra sin
preguntar. Tampoco cuenta como cambio lo que trae la Work Order al comenzar
una prueba, ni un campo que modificaste y dejaste como estaba.

**Cambiar de base de datos.** Al elegir otra base en *Ajustes → Base de datos*,
todas las bitácoras pasan a leer y guardar en la nueva.

> Antes, Torsión y Quasi seguían en la base anterior hasta reiniciar la app,
> sin nada que lo delatara, porque las demás bitácoras sí cambiaban.
...
>
> La **013 (pieza movida durante un mantenimiento)** está escrita y probada
> sobre copias —dos pasadas seguidas, totales por tabla idénticos— y también se
> aplica sola, con su respaldo previo, la próxima vez que se abra la app.
> Agrega la columna `released_date` a `rig_maintenance_samples`. Solo rellena la
> de las piezas de un mantenimiento **abierto** que ya tienen banco otra vez, y
> les pone la fecha del día en que corre, porque no hay forma de saber cuándo se
> movieron. Los mantenimientos cerrados no se tocan.
...
La misma suite sigue a una pieza que se lleva a otro banco con el suyo todavía
parado:
- deja de estar detenida y su chip y su celda toman el color del banco nuevo;
- la pieza que se quedó esperando sigue en rojo;
- la prueba deja de sumar días detenida;
- al cerrar el mantenimiento no se la devuelve.

También comprueba que:
- los tres caminos que abren el formulario de fatiga (bitácora, *Ocupación de
  rigs* y *Work Orders*) lo hacen **sin los bancos parados**;
- una pieza que ya estaba en uno conserva su valor;
- la migración 013 libera lo que ya se había movido sin cambiar nada en una
  segunda pasada.

`tests/test_form_fit.py` simula la pantalla de un portátil (680 px útiles) y
mide los formularios de Fatiga y Rotary:
- el mínimo que exigen cabe en 768 px;
- el botón de guardar queda entero dentro del formulario;
- lo que no cabe se alcanza desplazando;
- en una pantalla grande se ven enteros y sin barra.

También comprueba que:
- la rueda sobre un combo o una fecha sin foco desplaza el formulario sin
  cambiar el valor, y con foco sí lo cambia;
- la lista de clientes abierta no pasa de 10 filas, con filas de al menos 26 px;
- Work Order, Torsión y Quasi siguen la misma estructura: aun abiertos a su
  alto mínimo, el botón de guardar se ve entero, no queda hueco encima de los
  botones y la rueda sin foco no cambia combos ni fechas.

`tests/test_concurrent_edits.py` simula dos equipos, «ana» y «luis», sobre una
copia de la base:
- **Campos distintos:** se combinan y el historial atribuye a cada uno solo lo
  suyo. Lo comprueba en Fatiga, Rotary, Torsión y en una Work Order que otro
  equipo comenzó mientras se corregía.
- **El mismo campo:** guardar se detiene sin escribir nada. *Guardar los míos*
  y *Dejar los guardados* resuelven solo ese campo y respetan lo demás. Un
  registro eliminado se avisa.
- **Desde los formularios:** el aviso dice el campo, los dos valores y quién
  guardó el otro; *Volver al formulario* no guarda nada.
- **Cambios sin guardar:** en los cuatro formularios, cancelar un formulario
  recién abierto no pregunta, uno con cambios sí, y la X de la ventana pasa
  por el mismo camino.
- **Cambiar de base:** revisa todas las pantallas buscando alguna que conserve
  la base anterior. Redirige `settings.json` a un archivo temporal para no
  tocar el real.

`tests/test_capture_aids.py` comprueba las ayudas de captura:
- **Campos que faltan:** guardar sin datos no abre ningún cuadro y marca a la
  vez cada campo con problema. La marca roja **se mide en píxeles** del borde.
  Corregir un campo le quita la marca y actualiza el aviso. Finalizar marca
  cada dato faltante en su pieza, sin pedir lo que no se exige, y la vista baja
  hasta el primero aunque esté al fondo. Lo revisa en los cinco formularios.
- **Capturar ciclos:** solo se editan las piezas que corren; guarda solo esos
  ciclos sin pisar lo que otro equipo guardó; un campo vacío conserva la
  lectura; el botón se enciende con una fila seleccionada y no existe en
  *Finalizadas*.
- **Movida el:** aparece solo al darle otro banco a una pieza detenida, limita
  las fechas al periodo del mantenimiento, guarda el día capturado y los días
  detenida terminan ahí.
