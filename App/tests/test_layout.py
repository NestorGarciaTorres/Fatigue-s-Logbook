...
from app.models import FATIGUE, FINISHED, QUASI, ROTARY, TORSION, Requester, WorkOrder
...
# Solo las dos bitacoras de 46 columnas. Torsion y Quasi tienen siete y,
# maximizadas, usaban 870 px de 2528: dos tercios de pantalla en blanco.
for key in ("fatigue", "rotary"):
...
check("solo las bitacoras anchas van a pantalla completa",
      FULL_WIDTH_PAGES == {"fatigue", "rotary"},
...
check("hay un rotulo por grupo",
      ...
      and any("GESTIÓN" in t for t in etiquetas), str(etiquetas))
...
check("quedan tres graficas", len(graficas) == 3, str(len(graficas)))
...
# Las piezas por banco van en un panel de renglones, no en una grafica:
# con dieciseis bancos y 1,594 piezas contra 6, las barras chicas no
# llegaban a un pixel.
check("y esta el panel de piezas por banco", dash.rig_usage in vistas)

tipo_chart = next(v.chart() for v in graficas if "tipo de ensayo" in v.chart().title())
...
# Una barra por tipo, cada una con su color y su nombre en la leyenda.
# Antes era una sola serie verde con la leyenda "Finalizadas", que solo
# repetia el titulo de la grafica.
check("una barra por tipo de ensayo",
      conjuntos == ["Fatiga", "Rotary", "Torsión", "Quasi"], str(conjuntos))
check("cada tipo con el color con el que aparece en el menu",
      [bs.color().name().upper() for bs in series.barSets()]
      == [c.accent.upper() for c in (FATIGUE, ROTARY, TORSION, QUASI)],
      str([bs.color().name() for bs in series.barSets()]))

valores = [bs.at(0) for bs in series.barSets()]
