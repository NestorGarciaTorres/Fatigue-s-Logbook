...
# Se cambia el numero de piezas y luego se cierra el formulario. Sin esto, la
...
posiciones = {
    ...
    "Solicitante": celda(general, nuevo.requester),
...
# Cuatro filas para ocho campos: el solicitante agrego una. Lo que se mide
# no es el numero sino que sigan repartidos en rejilla y no apilados uno por
# fila, que es de donde se venia.
check("caben en cuatro filas en vez de en una por campo",
      len(filas) == 4, str(sorted(filas)))
check("la primera fila lleva tres campos distintos",
      len({c for f, c in posiciones.values() if f == 0}) == 3)
ultima = max(filas)
check("comentarios ocupa su propia fila, la ultima",
      posiciones["Comentarios"][0] == ultima
      and [n for n, (f, _) in posiciones.items() if f == ultima] == ["Comentarios"],
      str([n for n, (f, _) in posiciones.items() if f == ultima]))

alto_general = nuevo.batch.parentWidget().sizeHint().height()
print(f"      alto de 'Datos generales': {alto_general} px")
# El limite sale de lo que mediria la lista vertical de la que se venia: un
# campo por fila. Asi la comprobacion sigue valiendo cuando se agregue otro
# campo, en vez de tener que reajustar un numero a mano.
apilado = len(posiciones) * nuevo.batch.sizeHint().height()
check("el bloque es mas bajo que un campo por fila",
      alto_general < apilado, f"{alto_general} px contra {apilado} px apilado")
