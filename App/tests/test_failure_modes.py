...
# Cuantas piezas usaban ya estos modos antes de que la prueba agregue las
# suyas. Se esperaba 'Desgaste' en una sola: la del registro de prueba. El
# 10/09/2026 el laboratorio capturo dos piezas con 'Desgaste' y la
# comprobacion fallo sin que nada estuviera mal.
desgaste_antes = repository.failure_mode_usage("Desgaste")
fuga_antes = repository.failure_mode_usage("Fuga")
...
print(f"      {items}")
# El primer elemento es la opcion que vacia el combo: hace falta para poder
# borrar un modo ya elegido, y no es un modo del catalogo.
check("la primera opcion es la de vaciar", combo.itemData(0) == "", items[0])
check("ofrece todos los modos activos",
      items[1:] == catalogs.failure_modes(), str(items[1:]))
check("es una lista cerrada", not combo.isEditable())
check("abre vacio en un registro nuevo", combo_value(combo) == "",
      repr(combo_value(combo)))
...
check("al editar, carga el modo capturado",
      combo_value(editing.failure_modes[0]) == "Desgaste",
      combo_value(editing.failure_modes[0]))
check("al editar, la pieza sin modo queda vacia",
      combo_value(editing.failure_modes[2]) == "",
      repr(combo_value(editing.failure_modes[2])))
...
# Desde la migracion 008 el resultado tiene columna propia, entre el banco y
# los ciclos: son cuatro por muestra, no tres.
# El indice se deriva de la propia cabecera y no se escribe a mano: las
# columnas fijas de la izquierda han crecido dos veces ya (el resultado en la
# migracion 008, el solicitante en la 012) y cada vez habia que renumerar esto.
primera = headers.index("Test Rig 1")
check("el orden por muestra es rig, resultado, ciclos, modo",
      headers[primera:primera + 4] == ["Test Rig 1", "Resultado 1",
                                       "Ciclos 1", "Modo falla 1"],
      str(headers[primera:primera + 4]))
...
# Por su rotulo, no por ser la ultima fila: debajo puede ir la nota que
# explica el naranja de las piezas suspendidas.
totals_row = next(f for f in range(1, sheet.max_row + 1)
                  if sheet.cell(row=f, column=1).value == "TOTALES")
...
check("cuenta cuantas muestras usan cada modo",
      ...
      == str(desgaste_antes + 1),
      f"{tab.table.item(labels.index('Desgaste'), 2).text()} "
      f"(habia {desgaste_antes} y la prueba agrega 1)")
check("un modo sin uso cuenta cero",
      tab.table.item(labels.index("Pandeo"), 2).text() == "0")
check("el conteo de uso ve Fatiga y Rotary",
      repository.failure_mode_usage("Fuga") == fuga_antes + 1,
      f"{repository.failure_mode_usage('Fuga')} "
      f"(habia {fuga_antes} y la prueba agrega 1)")
