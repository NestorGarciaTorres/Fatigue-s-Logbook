...
from app.models import (
    ONGOING, SAMPLE_SLOTS, WO_PENDING, WO_STARTED, FatigueSample,
    FatigueTest, Requester, WorkOrder,
)
...
# Las cuatro bitacoras comienzan su prueba desde aqui. Torsion y Quasi eran
# las que faltaban: su orden se registraba pero habia que ir a la bitacora a
# dar de alta la prueba a mano.
check("una WO de torsion tambien se puede comenzar",
      repo.get(torsion_id).can_start)
...
check("la WO de torsion ofrece su boton de comenzar",
      acciones["990002STO01"] == ("QPushButton", True),
...
# Torsion es la unica que conserva el alta directa: ahi llega la pieza y se
# corre sin orden previa. Se comprueba el boton, no el atributo: existe en las
# dos paginas y en Quasi vale None, asi que hasattr() daria verde por el
# motivo equivocado.
check("pero Torsion conserva el suyo",
      window.pages["torsion"].new_button is not None)
check("y Quasi ya no: su prueba nace de una Work Order",
      window.pages["quasi"].new_button is None)
...
print("=== 7b. Corregir el Test Batch al comenzar la prueba ===")

# El formulario deja corregir el Test Batch de la orden --para eso se dejo
# editable-- y lo que vale es con que se guardo la prueba. Si la orden se
# quedara con el viejo pasarian dos cosas: la orden y su prueba dirian numeros
# distintos, y el viejo --que no existe en ninguna bitacora-- bloquearia para
# siempre el alta de una orden nueva con ese numero.
codigo = context.catalogs.codes("fatigue")[0]
PEDIDO = f"991001{codigo}01"
CORREGIDO = f"991002{codigo}01"

corregible = repo.create(
    WorkOrder(test_type="fatigue", test_batch=PEDIDO, customer="AUDI",
              qty_samples=1, requester="Ana Torres"),
    AUTHOR,
)
nacida = context.fatigue.create(
    FatigueTest(test_batch=CORREGIDO, customer="AUDI", start_date=date.today(),
                qty_samples=1, test_status=ONGOING,
                samples=[FatigueSample() for _ in range(SAMPLE_SLOTS)]),
    AUTHOR,
)
repo.mark_started(corregible, nacida, AUTHOR)

orden = repo.get(corregible)
check("la orden se queda con el Test Batch de su prueba",
      orden.test_batch == CORREGIDO, orden.test_batch)
check("y sigue enlazada al registro que la origino",
      orden.started_test_id == nacida)
check("el numero que se descarto queda libre otra vez",
      not repo.batch_exists(PEDIDO),
      f"{PEDIDO} sigue ocupado por una orden que no lo usa")
check("el que si se uso sigue ocupado", repo.batch_exists(CORREGIDO))
cambios = [e for e in context.audit.for_record("work_orders", corregible)
           if e.field == "test_batch"]
check("la correccion queda en el historial", len(cambios) == 1,
      str([(e.old_value, e.new_value) for e in cambios]))
if cambios:
    check("diciendo de que numero a cual",
          (cambios[0].old_value, cambios[0].new_value) == (PEDIDO, CORREGIDO),
          f"{cambios[0].old_value} -> {cambios[0].new_value}")

# Y si no se corrige nada, la orden no se toca.
SIN_CAMBIO = f"991003{codigo}01"
igual = repo.create(
    WorkOrder(test_type="fatigue", test_batch=SIN_CAMBIO, customer="AUDI",
              qty_samples=1, requester="Ana Torres"),
    AUTHOR,
)
misma = context.fatigue.create(
    FatigueTest(test_batch=SIN_CAMBIO, customer="AUDI", start_date=date.today(),
                qty_samples=1, test_status=ONGOING,
                samples=[FatigueSample() for _ in range(SAMPLE_SLOTS)]),
    AUTHOR,
)
repo.mark_started(igual, misma, AUTHOR)
check("sin correccion, no se anota ningun cambio de Test Batch",
      not [e for e in context.audit.for_record("work_orders", igual)
           if e.field == "test_batch"])
