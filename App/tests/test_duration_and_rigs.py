...
# La pagina las expone con cards(): antes se sacaban del layout a mano, y
# el layout dejo de ser uno solo al agrupar los bancos por bitacora.
cards = page.cards()
...
# Tres estados desde que hay mantenimiento, no dos: un banco parado no esta
# ocupado --lo que quede anotado en el son piezas que ya terminaron-- ni esta
# libre, porque no se puede poner nada a correr en el.
parados = [c for c in cards if c.maintenance is not None]
busy = [c for c in cards if c.occupants and c.maintenance is None]
free = [c for c in cards if not c.occupants and c.maintenance is None]
print(f"     ocupados: {sorted(c.rig.name for c in busy)}")
print(f"     libres:   {sorted(c.rig.name for c in free)}")
if parados:
    print(f"     parados:  {sorted(c.rig.name for c in parados)}")
check("hay rigs ocupados y libres", busy and free,
      f"{len(busy)} ocupados, {len(free)} libres, {len(parados)} parados")
...
check("las tarjetas coinciden con las tarjetas resumen",
      ...
      and page.card_maintenance.value_label.text() == str(len(parados)),
      f"resumen {page.card_busy.value_label.text()}/"
      f"{page.card_free.value_label.text()}/"
      f"{page.card_maintenance.value_label.text()}, tarjetas "
      f"{len(busy)}/{len(free)}/{len(parados)}")
check("los tres estados suman el catalogo",
      len(busy) + len(free) + len(parados) == len(cards),
      f"{len(busy)} + {len(free)} + {len(parados)} de {len(cards)}")
