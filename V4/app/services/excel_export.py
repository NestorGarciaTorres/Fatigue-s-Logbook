...
BASE_HEADERS = ["ID", "Test Batch", "Cliente", "Inicio", "Piezas", "Comentarios"]
...
def export_fatigue_ongoing(
    ...
    for test in tests:
        ...
        for sample in test.samples:
            row += [
                SUSPENDED_LABEL if not test.is_finished and sample.is_suspended
                else "" if sample.rig == EMPTY_RIG else sample.rig,
                sample.result or "",
    ...
    if any(s.is_suspended for t in tests if not t.is_finished
           for s in t.samples):
        note_row = total_row + 2
        sheet.merge_cells(start_row=note_row, start_column=1, end_row=note_row,
                          end_column=len(BASE_HEADERS))
        nota = sheet.cell(row=note_row, column=1, value=SUSPENDED_NOTE)
        nota.fill = SUSPENDED_FILL
