...
def validate_complete_for_finish(
    ...
    missing = [caption for caption, value in general.items() if _blank(value)]
    if missing:
        problems.append("Datos generales: " + ", ".join(missing))

    for number, fields in samples:
        missing = [caption for caption, value in fields.items() if _blank(value)]
        if missing:
            problems.append(f"Pieza {number}: " + ", ".join(missing))
