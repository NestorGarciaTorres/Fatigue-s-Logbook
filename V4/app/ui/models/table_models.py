...
from app.models import EMPTY_RIG, FINISHED, ONGOING, SAMPLE_SLOTS
...
class BaseTestTableModel(QAbstractTableModel):
    ...
    @staticmethod
    def _closed_days(record) -> tuple[int | None, str | None, str | None]:
...
class FatigueTableModel(BaseTestTableModel):
    ...
    def rig_cell_color(self, record, column: int, value) -> str | None:
        ...
        if color or record.is_finished:
            return color
        # Sin banco: puede ser una ranura que nadie uso o una pieza retirada
        # del banco a medio correr. Lo dice la muestra de esa misma columna.
        sample = record.samples[(column - self._first_sample) // 4]
